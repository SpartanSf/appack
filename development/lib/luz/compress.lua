local libFolder = (function()
    local info = debug.getinfo(2, "S")
    local path = info and info.source and info.source:sub(1, 1) == "@" and info.source:sub(2) or nil
    return path and fs.getDir(path) or nil
end)()

local LibDeflate = require(fs.combine(libFolder, "LibDeflate"))
local maketree = require(fs.combine(libFolder, "maketree"))
local lz77 = require(fs.combine(libFolder, "lz77"))
local token_encode_map = require(fs.combine(libFolder, "token_encode_map"))
